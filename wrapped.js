// Full-page Wrapped script - Story format
let currentSlide = 0;
let totalSlides = 8;
let stats = null;

document.addEventListener('DOMContentLoaded', () => {
  console.log('[DoorDash Wrapped] Full-page view loaded');
  ...
  if (stats.deliveryLocations.uniqueCount === 0) {
    ...
    stats.deliveryLocations.topLocations.forEach((location, index) => {
      ...
      if (displayAddress.length > 45) {
        displayAddress = displayAddress.substring(0, 45) + '...';
      }
      li.innerHTML = `
        <span class="list-rank">${index + 1}</span>
        <span class="list-name">${escapeHtml(displayAddress)}</span>
